...
<div class="flex flex-col gap-6">
    <div class="flex flex-col gap-1">
        <div class="flex items-center gap-3">
            ...
            <div>
                <h1 class="text-2xl font-semibold tracking-tight">Drużyny</h1>
                <p class="text-sm text-muted-foreground">
                    {#if cities.length > 0}
                        {cities.length} {cities.length === 1 ? 'miasto' : 'miast'}
                    {:else}
                        Wybierz miasto, aby zobaczyć drużyny
                    {/if}
                </p>
            </div>
        </div>
    ...
    {:else if cities.length === 0}
        <div class="rounded-lg border border-dashed p-12 text-center">
            ...
            <p class="text-sm text-muted-foreground/70 mt-1">Drużyny pojawią się po ich zarejestrowaniu.</p>
        </div>
    {:else}
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {#each cities as city (city.id)}
                <button
                        ...
                        onclick={() => goto(`/dashboard/teams/city/${encodeURIComponent(city.name)}`)}
                >
                    <Card.Root
