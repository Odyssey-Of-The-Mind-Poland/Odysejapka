<script lang="ts">
    ...
    let {cityId, userId, userName}: { cityId: number; userId: number; userName: string } = $props();
    ...
    async function fetchCredentials() {
        ...
        error = null;
        try {
            credentials = await apiFetch<Credentials>(`/api/v1/spontan/user/${cityId}/${userId}/credentials`);
        } catch (e) {
            error = 'Brak danych logowania dla tego użytkownika.';
            credentials = null;
        } finally {
            loading = false;
        }
...
<Dialog.Root open={open} onOpenChange={handleOpen}>
    ...
    <Dialog.Content class="sm:max-w-md">
        <Dialog.Header>
            <Dialog.Title>Dane logowania — {userName}</Dialog.Title>
            <Dialog.Description>
                Poświadczenia użytkownika spontan.
            </Dialog.Description>
        </Dialog.Header>
