    ...
    <div class="flex flex-col gap-1">
        <div class="flex items-center gap-3">
            <div class="flex items-center justify-center size-10 rounded-lg bg-primary/10">
                <IconTrophy class="size-5 text-primary"/>
            </div>
            <div>
                <h1 class="text-2xl font-semibold tracking-tight">Konkursy</h1>
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {#each cities as city (city.id)}
                <button
                        ...
                        onclick={() => goto(`/dashboard/competitions/${city.id}/dt`)}
                >
                                ...
                                <div class="flex items-center gap-1">
                                    <RequirePermission role="ADMINISTRATOR">
                                        <Button
                                                ...
                                                onclick={(e) => { e.stopPropagation(); deleteCityMutation.mutate({cityId: city.id}); }}
