<script lang="ts">
    ...
    import type {SpontanRoom} from "../types";
    ...
    type Props = {
        ...
        onsave: (roomId: number, startTime: string, slotDurationMinutes: number, count: number) => void;
    };

    let {open = $bindable(false), rooms, saving, preselectedRoomId = '', onsave}: Props = $props();
    ...
    let slotDuration = $state(15);
    let slotCount = $state(10);

    let canSave = $derived(selectedRoomId !== '' && startTime && slotDuration > 0 && slotCount > 0);

    function handleSave() {
        if (!canSave) return;
        onsave(Number(selectedRoomId), startTime, slotDuration, slotCount);
    }
...
<Dialog.Root bind:open>
    <Dialog.Content class="sm:max-w-md">
        <Dialog.Header>
            <Dialog.Title>Generuj sloty spontaniczne</Dialog.Title>
            <Dialog.Description>
                Ustaw pokoj, godzine rozpoczecia, czas trwania i liczbe slotow
            </Dialog.Description>
        </Dialog.Header>

        <div class="flex flex-col gap-4 py-4">
            <div>
                <label class="text-sm font-medium mb-1 block">Pokoj</label>
                <Select.Root type="single" bind:value={selectedRoomId}>
                    <Select.Trigger>
                        {rooms.find(r => String(r.id) === selectedRoomId)?.name || 'Wybierz pokoj...'}
                    </Select.Trigger>
                    <Select.Content>
