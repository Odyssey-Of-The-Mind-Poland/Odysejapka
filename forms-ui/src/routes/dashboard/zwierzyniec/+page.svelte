<script lang="ts">
	...
	onMount(() => {
		setBreadcrumbs([{ name: 'Zwierzyniec', href: '/dashboard/zwierzyniec' }]);
	});
</script>
...
<div class="flex flex-col gap-6">
	<div class="flex flex-col gap-1">
		<div class="flex items-center gap-3">
			<div class="flex items-center justify-center size-10 rounded-lg bg-primary/10">
				<IconTrees class="size-5 text-primary" />
			</div>
			<div>
				<h1 class="text-2xl font-semibold tracking-tight">Zwierzyniec</h1>
				<p class="text-sm text-muted-foreground">
					RAK, GAD, SAK, Fixer — narzędzia administracyjne
				</p>
			</div>
		</div>
	</div>
	...
	{:else if cities.length === 0}
		<div class="rounded-lg border border-dashed p-12 text-center">
			...
			<p class="text-sm text-muted-foreground/70 mt-1">Wybierz miasto z listy konkursów.</p>
		</div>
	{:else}
		<div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
			{#each cities as city (city.id)}
				<button
					...
					onclick={() => goto(`/dashboard/zwierzyniec/${city.id}/rak`)}
				>
					<Card.Root class="cursor-pointer transition-all hover:shadow-md hover:border-primary/30">
						<Card.Header>
